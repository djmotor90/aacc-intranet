...
export async function POST(req: Request, ctx: { params: Promise<{ taskId: string }> }) {
  ...
  if (typeof folderRaw === "string" && folderRaw.trim()) {
    const [folder] = await db
      ...
      .where(and(eq(attachmentFolders.id, folderRaw), eq(attachmentFolders.taskId, taskId)));
    if (!folder) {
  ...
  // "embed" = pasted/dropped into TipTap (comment or description). Still stored
  // as an attachment for serving, but must not show as "added attachment" in
  // the activity feed — the comment body already carries the image.
