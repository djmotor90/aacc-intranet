...
export function isImageFile(file: File): boolean {
  ...
  // Some OS / clipboard sources omit MIME — fall back to extension.
  return /\.(png|jpe?g|gif|webp|heic|bmp|svg)$/i.test(file.name || "");
}
...
async function postUpload(
  ...
  // Editor paste/drop/insert — store the file but do NOT emit "added attachment"
  // activity (that would clutter the feed when commenting with images).
  formData.set("purpose", "embed");
...
export async function uploadTaskFile(
  ...
  return postUpload(`/api/tasks/${taskId}/attachments`, file);
}

/** Upload an image/file embed for a Docs page. */
export async function uploadDocPageFile(
  pageId: string,
  file: File,
): Promise<UploadedFile> {
