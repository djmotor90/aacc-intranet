...
function ancestorKeysForPath(taskNavTree: TaskNavTreeItem[], pathname: string): string[] {
  ...
  for (const space of taskNavTree) {
    ...
    keys.push(tasksRootCollapseKey(space.moduleId));
...
const SpaceDropRow = forwardRef<HTMLElement, SpaceDropRowProps>(function SpaceDropRow(
  ...
  const inner = (
    ...
    >
      <EntityIcon icon={space.icon} color={space.color} fallback="space" size="xs" />
...
export function SidebarNav({
  ...
  isAdmin?: boolean;
  /** Icon-only rail (no labels, no space tree). */
  collapsed?: boolean;
}) {
    ...
    <DndContext id="sidebar-tree-dnd" sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd}>
      <nav className="flex flex-col gap-1">
        {items.map((item) => {
          ...
          const onModuleSpace = moduleSpaces.some(
            (s) =>
              pathname === `/tasks/${s.slug}` || pathname.startsWith(`/tasks/${s.slug}/`),
          );
          ...
          const isActive =
            item.href === "/"
              ? pathname === "/"
              : isWorkspaceApp
                ? onModuleHome || onModuleSpace || onTasksGlobal
                : onModuleHome;

          const rootKey = tasksRootCollapseKey(item.moduleId ?? item.href);
          const rootExpanded = isExpanded(rootKey);
          // Do not require isActive — otherwise the chevron only rotates until the
          // user first navigates into the module (click Tasks / IT label).
          const treeAvailable =
            !collapsed && isWorkspaceApp && moduleSpaces.length > 0;
          const showTree = treeAvailable && rootExpanded;

          if (collapsed) {
            return (
              <div key={item.href + (item.moduleId ?? "")} className="flex justify-center">
                <Link
                  href={item.href}
                  title={item.label}
          ...
          const linkEl = (
            ...
            >
              ...
              {isWorkspaceApp && moduleSpaces.length > 0 && (
                <button
                  type="button"
                  aria-label={rootExpanded ? `Collapse ${item.label}` : `Expand ${item.label}`}
          ...
          return (
            <div key={item.href + (item.moduleId ?? "")}>
              {isWorkspaceApp ? (
                <TasksRootContextMenu
                  isAdmin={isAdmin}
