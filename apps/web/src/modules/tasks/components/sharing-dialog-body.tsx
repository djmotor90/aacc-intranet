            ...
            <div key={m.id} className="flex items-center justify-between gap-2 py-1">
              ...
              {isPrivate ? (
                <span className="text-xs text-muted-foreground">
                  {m.role === "owner" ? "admin (always has access)" : "no access"}
                </span>
              ) : m.role === "owner" ? (
                <span className="text-xs text-muted-foreground">admin (can&apos;t be removed)</span>
              ) : (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground">
                    {m.role === "member" ? "Member" : m.role === "guest" ? "Guest" : m.role}
                  </span>
                  {lockAction && (
