  ...
  return (
    ...
    >
      {renaming ? (
        ...
        >
          <ViewTypeIcon type={view.type} className="size-3.5 shrink-0 opacity-70" />
...
export function ListViewTabs({
  ...
  // Adjust local views when the server prop changes (React "adjust state
  // during render" pattern — no post-paint sync effect needed).
  const [prevInitialViews, setPrevInitialViews] = useState(initialViews);
  if (prevInitialViews !== initialViews) {
    setPrevInitialViews(initialViews);
  ...
  function create(type: "table" | "board" | "form" | "grid") {
    startTransition(async () => {
      try {
        let id: string;
        if (type === "form") {
          const created = await createFormView({ listId });
          id = created.viewId;
        } else {
      ...
      <DragOverlay dropAnimation={null}>
        {activeDragView ? (
          <div className="flex h-8 items-center gap-1.5 rounded-md border bg-background px-2.5 text-sm font-medium shadow-md">
            <ViewTypeIcon type={activeDragView.type} className="size-3.5 opacity-70" />
