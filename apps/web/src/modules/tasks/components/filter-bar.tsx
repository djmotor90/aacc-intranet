...
function GroupByPopover({
  ...
  groupBy: groupByProp = "",
  /** Soft group-by (no full page navigation). Prefer this when provided. */
  onGroupByChange,
}: {
  ...
  // Prefer controlled prop when soft group-by is active; else URL / prop fallback.
  const current = onGroupByChange
    ? (groupByProp ?? "")
  ...
  function select(value: string) {
    ...
    // Legacy full navigation path (e.g. board header without shell).
    const params = new URLSearchParams(searchParams.toString());
...
export function FilterBar({
  ...
  groupBy?: string;
  /** Soft group-by without full page reload. */
  onGroupByChange?: (value: string) => void;
}) {
