...
export function TagPicker({
  ...
  // Adjust optimistic state when the server props change (React "adjust state
  // during render" pattern — no post-paint sync effect needed).
  const [prevSelected, setPrevSelected] = useState(selectedTags);
  const [prevSpaceTags, setPrevSpaceTags] = useState(spaceTags);
  if (prevSelected !== selectedTags) {
    setPrevSelected(selectedTags);
    setLocalSelected(selectedTags);
  }
  if (prevSpaceTags !== spaceTags) {
    setPrevSpaceTags(spaceTags);
