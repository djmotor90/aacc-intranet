...
export const customFieldType = pgEnum("custom_field_type", [
  ...
  "email",
  "phone",
  /** Single-select palette: options are `{ id, label, color }` hex swatches. */
  "color",
  /** File(s) — value is array of { attachmentId, fileName, … } in tasks.custom_fields. */
  "file",
]);
...
export const lists = pgTable(
  "lists",
  {
    ...
    isPrivate: boolean("is_private").notNull().default(false),
    /**
     * Core (built-in) fields required when creating a task on this list.
     * Values: "priority" | "dueDate" | "startDate" | "assignees" | "description" | "tags"
     * Title is always required; status uses the list default on create.
     */
    requiredCoreFields: jsonb("required_core_fields").notNull().default(sql`'[]'::jsonb`),
    /**
     * When false, users cannot create new subtasks on this list.
     * Existing subtasks stay linked and editable (no data loss).
     */
    subtasksEnabled: boolean("subtasks_enabled").notNull().default(true),
    /**
     * Turns on the Secrets vault tab on tasks in this list. Off by default
     * (opt-in, unlike subtasks) since it deals with sensitive data. Disabling
     * only blocks new secrets — existing encrypted ones are never deleted.
     */
...
export const tasks = pgTable(
  "tasks",
  {
    ...
    listId: uuid("list_id")
      ...
      .references(() => lists.id),
    /**
     * When set, this task is a subtask of another task in the same list.
    ...
    isArchived: boolean("is_archived").notNull().default(false),
    /** Soft-delete — set by the "Delete" action; task is restorable from Trash until purged. */
    deletedAt: timestamp("deleted_at", { withTimezone: true }),
    ...timestamps,
  },
  (t) => [
...
export const taskFollowers = pgTable(
  "task_followers",
  {
    ...
    userId: uuid("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    /** How they started watching: assigned | manual */
...
/** Nested folders for task file attachments (root = parent_folder_id null). */
export const attachmentFolders = pgTable(
  ...
  (t) => [
    index("attachment_folders_task_idx").on(t.taskId),
    index("attachment_folders_parent_idx").on(t.parentFolderId),
  ],
);
...
export const attachments = pgTable(
  "attachments",
  {
    ...
    taskId: uuid("task_id")
      .notNull()
      .references(() => tasks.id, { onDelete: "cascade" }),
    /** null = root of the task attachment browser */
    folderId: uuid("folder_id").references(() => attachmentFolders.id, { onDelete: "set null" }),
...
export const notifications = pgTable(
  "notifications",
  {
    ...
    /**
     * Last time the recipient opened the in-notification reply thread.
     * Used to show a "new messages in thread" badge without reopening.
     */
    threadViewedAt: timestamp("thread_viewed_at", { withTimezone: true }),
...
export const attachmentFoldersRelations = relations(attachmentFolders, ({ one, many }) => ({
  task: one(tasks, { fields: [attachmentFolders.taskId], references: [tasks.id] }),
  parent: one(attachmentFolders, {
    fields: [attachmentFolders.parentFolderId],
    references: [attachmentFolders.id],
    relationName: "attachment_folder_tree",
  }),
  children: many(attachmentFolders, { relationName: "attachment_folder_tree" }),
  attachments: many(attachments),
}));
